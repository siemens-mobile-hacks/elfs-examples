...
// Функция, которая запрашивает отрисовку буфера на LCD
// В прошивке уже есть готовая функция, но её нет в swilib
static void redraw_lcd_layer(LCDLAYER *layer) {
...
static void gui_oncreate(GUI *data, malloc_func_t malloc_fn) {
	...
	// Рисуем напрямую в скринбуфер MMI
	LCDLAYER *layer = GetLCDLayerList()[LCDLAYER_MMI_ID];
	
	uint16_t *bytes = layer->buffer;
	for (int i = 0; i < ScreenW() * ScreenH(); i++)
		bytes[i] = i;
	
	// Инвалидируем область экрана
	layer->invalidate.x = 0;
	layer->invalidate.y = 0;
	layer->invalidate.x2 = ScreenW() - 1;
	layer->invalidate.y2 = ScreenH() - 1;
	
	// Запрашиваем отрисовку буфера в LCD
	redraw_lcd_layer(layer);
}
